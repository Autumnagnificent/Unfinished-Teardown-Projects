...
function AutoRndVec(length, precision)
	...
	return AutoVecRescale(v, length)
end
...
---Returns a string to sperated by spaces from a vector
---@param vec any
---@param round number
---@return string
...
---Returns a vector to from a string sperated by spaces
---@param str any
---@return table
function AutoXMLToVec(str)
	local splits = AutoSplit(str, ' ')
	local values = {}
	for _, s in pairs(splits) do values[#values + 1] = tonumber(s) end
	return values
end
...
function AutoTransformUp(t)
	return TransformToParentVec(t, Vec(0, 1, 0))
end
...
function AutoInvert(bool, invert)
	invert = AutoDefault(invert, false)
	return invert and (not bool) or bool
end
